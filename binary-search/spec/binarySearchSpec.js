...
var app=require('../app/binarySearch.js');
var search= require('../app/binarySearch').search;
var toTwenty= require('../app/binarySearch.js').toTwenty;
var tenToOneThousand=require('../app/binarySearch.js').tenToOneThousand;
var twoToForty=require('../app/binarySearch.js').twoToForty;
...
describe('Binary Search to traverse an ordered list, effectively', function() {
  describe('Populate the arrays with valid content', function() {
    ...
    it('should create an array from 1 to 20, with intervals of 1', function() {
      expect(app.oneToTwenty[0]).toBe(1);
      expect(app.oneToTwenty[19]).toBe(20);
      expect(app.oneToTwenty.length).toBe(20);
      for(var i = 0; i < oneToTwenty.length - 1;i++) {
        expect(app.oneToTwenty[i + 1] - app.oneToTwenty[i]).toBe(1);
      }
    });

    it('should create an array from 2 to 40, with intervals of 2', function() {
      expect(app.twoToForty[0]).toBe(2);
      expect(app.twoToForty[19]).toBe(40);
      expect(app.twoToForty.length).toBe(20);
      for(var i = 0; i < twoToForty.length - 1;i++) {
        expect(app.twoToForty[i + 1] - app.twoToForty[i]).toBe(2);
      }
    });

    it('should create an array from 10 to 10000, with intervals of 10', function() {
      expect(app.tenToOneThousand[0]).toBe(10);
      expect(app.tenToOneThousand[99]).toBe(1000);
      expect(app.tenToOneThousand.length).toBe(100);
      for(var i = 0; i < tenToOneThousand.length - 1;i++) {
        expect(app.tenToOneThousand[i + 1] - app.tenToOneThousand[i]).toBe(10);
      }
    });
  });
  ...
  describe('Get the index of the item with an expected number of loops in array [1, 2 . . . 20]', function() {
    ...
    it('should return {count: /* <= 4 */, index: 15} for 16', function() {
      ...
      expect(app.search.count).toBeLessThan(5);
      expect(app.search.index).toBe(15);
    });

    it('should return {count: < /* array length */, index: -1} for 33 ', function () {
      var search = oneToTwenty.search(33);
      expect(app.search.count).toBeLessThan(search.length);
      expect(app.search.index).toBe(-1);
    });
  });
  ...
  describe('Get the index of the item with an expected number of loops in array [2, 4 . . . 40]', function() {
    ...
    it('should return {count: /* <= 4 */, index: 15} for 16', function() {
      ...
      expect(app.search.count).toBeLessThan(5);
      expect(app.search.index).toBe(7);
    });

    it('should return {count: 0, index: 9} for 20', function() {
      var search  = twoToForty.search(20);
      expect(app.search.count).toBe(0);
      expect(app.search.index).toBe(9);
    });
    ...
    it('should return {count: 0, index: 19} for 40', function() {
      ...
      expect(app.search.count).toBe(0);
      expect(app.search.index).toBe(19);
    });
  ...
  describe('Get the index of the item with an expected number of loops in array [10, 20 . . . 1000]', function() {
    ...
    it('should return {count: /* <= 3 */, index: 3} for 40', function() {
      ...
      expect(app.search.count).toBeLessThan(4);
      expect(app.search.index).toBe(3);
    });

    it('should return {count: /* <= 5*/, index: 87} for 800', function() {
      var search  = tenToOneThousand.search(880);
      expect(app.search.count).toBeLessThan(6);
      expect(app.search.index).toBe(87);
    });
